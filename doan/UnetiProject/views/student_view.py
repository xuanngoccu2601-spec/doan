...
COLOR_ACCENT = "#1abc9c"
COLOR_HOVER = "#16a085"
COLOR_BORDER_NEW = "#174260" 

# MÀU MỚI:
COLOR_BLUE_TITLE = "#00bfff"     
TEXT_COLOR_BLACK = "#000000"     

# MÀU ĐÃ SỬA CHO open_grading
COLOR_HOVER_LIGHTBLUE = "#c5e3f8" 
# THAY ĐỔI: Màu xanh đậm chính (fg_color) thành #3a8bbd
COLOR_BLUE_DARKER = "#3a8bbd"    
# Màu hover mới cho các nút xanh đậm (nhạt hơn #3a8bbd)
COLOR_BLUE_DARKER_HOVER = "#46a2c9" 
COLOR_RED_LIGHT = "#e74c3c"      
...
class StudentDashboard(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        def create_sidebar_btn(text, cmd, icon="🔹"):
            ...
            btn.pack(fill="x", padx=(40, 15), pady=5) 
        ...
        # --- ĐIỀU CHỈNH NÚT ĐĂNG XUẤT ---
        # Thay thế icon và đặt sang phải
    ...
    # =========================================================================
    # HÀM QUẢN LÝ CỬA SỔ CON
    # =========================================================================
    def destroy_previous_child_window(self):
        """Đóng hoàn toàn cửa sổ con đang hoạt động (nếu có)."""
        if self.active_child_window and self.active_child_window.winfo_exists():
            self.active_child_window.destroy()
            self.active_child_window = None

    def set_new_child_window(self, window):
        """Thiết lập cửa sổ con mới và gán protocol đóng."""
        window.protocol("WM_DELETE_WINDOW", lambda: (window.destroy(), setattr(self, 'active_child_window', None)))
        self.active_child_window = window
    ...
    def setup_treeview_style(self):
        # Đảm bảo nội dung bảng màu đen
        style = ttk.Style()
        style.theme_use("clam")
        style.configure("Treeview.Heading", font=("Roboto Medium", 13), background="#ecf0f1", foreground=COLOR_TEXT_MAIN, relief="flat")
        style.configure("Treeview", font=("Arial", 12), rowheight=35, background="white", fieldbackground="white", foreground=TEXT_COLOR_BLACK, borderwidth=0) 
        style.map("Treeview", 
                  background=[('selected', COLOR_HOVER_LIGHTBLUE)],
    ...
    def show_home_page(self):
        self.hide_all_pages()
        self.page_home.pack(fill="both", expand=True, padx=30, pady=(30, 30)) 
    ...
    def show_profile_page(self):
        ...
        self.page_profile.pack(fill="both", expand=True, padx=30, pady=(30, 30))
    ...
    def show_account_page(self):
        ...
        self.page_account.pack(fill="both", expand=True, padx=30, pady=(30, 30))

    # =========================================================================
    # TRANG CHỦ (GIỮ NGUYÊN HOVER & ICON)
    # =========================================================================
    def setup_home_page(self):
        ...
        grid.pack(fill="both", expand=True, pady=(60, 0)) 
        grid.columnconfigure((0, 1, 2), weight=1)

        # Định nghĩa màu Icon và Text 
        ICON_COLOR_LIGHTBLUE = "#00bfff" 
        ICON_COLOR_GREEN = "#2ecc71"     
        ICON_COLOR_ORANGE = "#ff8c00"    
        ICON_COLOR_BROWN = "#8b4513"     
        ICON_COLOR_YELLOW = "#ffd700"    
        ICON_COLOR_RED = "#e74c3c"       
        TEXT_COLOR_BLACK = "#000000"     
        
        buttons = [
            {"label": "Xem điểm\nRèn luyện", "icon": "📊", "cmd": self.open_view_scores, "icon_color": ICON_COLOR_LIGHTBLUE}, 
            {"label": "Đợt đánh giá", "icon": "📅", "cmd": self.open_periods, "icon_color": ICON_COLOR_YELLOW},
            {"label": "Thống kê lớp", "icon": "📈", "cmd": self.open_class_statistics, "icon_color": ICON_COLOR_ORANGE},
            {"label": "Lịch sử nộp", "icon": "🕒", "cmd": self.open_history, "icon_color": ICON_COLOR_BROWN},
            {"label": "Chấm điểm", "icon": "✍️", "cmd": self.open_grading, "icon_color": ICON_COLOR_GREEN},
            {"label": "Thông báo", "icon": "🔔", "cmd": self.open_notifications, "icon_color": ICON_COLOR_RED}
        ]
        
        for i, btn in enumerate(buttons):
            # 1. Tạo Frame hoạt động như nút
            card_frame = ctk.CTkFrame(
                grid, 
                fg_color="white", 
                corner_radius=15,
                border_width=2, 
                border_color=COLOR_BORDER_NEW, 
                height=150
            )
            ...
            # 4. Gán sự kiện nhấp chuột (click) cho Frame và tất cả Label con
            ...
            # 5. Gán hiệu ứng hover cho Frame và tất cả Label con
    ...
    # =========================================================================
    # HỒ SƠ CÁ NHÂN (NÚT QUAY LẠI Ở PHẢI DƯỚI)
    # =========================================================================
    def setup_profile_page(self):
        ...
        # Sử dụng fill="both" và expand=True để thẻ info_card chiếm hết không gian còn lại
        ...
        # Nút Quay lại (Đã di chuyển xuống dưới, bên phải)
        ctk.CTkButton(self.page_profile, text="← Quay lại", width=100, fg_color="transparent", 
                      ...
                      command=self.show_home_page).pack(anchor="e", pady=(20, 0))

    # =========================================================================
    # CÀI ĐẶT TÀI KHOẢN (NÚT QUAY LẠI Ở PHẢI DƯỚI)
    # =========================================================================
    def setup_account_page(self):
        ...
        # Sử dụng fill="both" và expand=True để thẻ menu_frame chiếm hết không gian còn lại
    ...
    # =========================================================================
    # CÁC HÀM POPUP (ĐÃ ÁP DỤNG XANH DA TRỜI VÀ NỘI DUNG ĐEN)
    # =========================================================================
    def confirm_logout(self):
        self.destroy_previous_child_window() 
    ...
    def open_guide(self):
        ...
        self.set_new_child_window(window)
        window.title("Hướng dẫn")
        self.center_window(window, 700, 500)
        window.transient(self.controller) 
        window.configure(fg_color="white")
        
        ctk.CTkLabel(window, text="HƯỚNG DẪN", font=("Roboto Medium", 20), text_color=COLOR_BLUE_TITLE).pack(pady=20)
    ...
    def open_change_password(self):
        ...
        def save():
            ...
            if self.db.change_password(self.current_user_id, old, new):
                messagebox.showinfo("Thành công", "Đổi mật khẩu thành công!", parent=window); window.destroy(); setattr(self, 'active_child_window', None)
    ...
    def open_view_scores(self):
        ...
        ctk.CTkLabel(window, text="BẢNG ĐIỂM RÈN LUYỆN", font=("Roboto Medium", 20), text_color=COLOR_BLUE_TITLE).pack(pady=20)
    ...
    def open_periods(self):
        ...
        ctk.CTkLabel(window, text="CÁC ĐỢT ĐÁNH GIÁ", font=("Roboto Medium", 20), text_color=COLOR_BLUE_TITLE).pack(pady=20)
        self.setup_treeview_style()
    ...
    def open_class_statistics(self):
        ...
        ctk.CTkLabel(window, text="THỐNG KÊ LỚP", font=("Roboto Medium", 22), text_color=COLOR_BLUE_TITLE).pack(pady=20)
    ...
    def open_history(self):
        ...
        ctk.CTkLabel(window, text="LỊCH SỬ NỘP PHIẾU", font=("Roboto Medium", 20), text_color=COLOR_BLUE_TITLE).pack(pady=20)
        self.setup_treeview_style()
    ...
    def open_grading(self):
        ...
        # Khởi tạo hoặc xóa các biến liên quan đến form
        self.selected_files = [] 
        ...
        header.pack(fill="x")
        # Tiêu đề Phiếu Đánh giá Rèn luyện thành màu Xanh da trời
        ctk.CTkLabel(header, text="PHIẾU ĐÁNH GIÁ RÈN LUYỆN", font=("Roboto Medium", 22), text_color=COLOR_BLUE_TITLE).place(relx=0.5, rely=0.5, anchor="center")
        ...
        # Tiêu đề Danh sách tiêu chí thành màu Xanh da trời mới
        ctk.CTkLabel(left_frame, text="DANH SÁCH TIÊU CHÍ ĐÁNH GIÁ", font=("Roboto Medium", 16), text_color=COLOR_BLUE_DARKER).pack(anchor="w", padx=20, pady=(15, 5))
        ...
        for title, items in data_sheet:
            ...
            for content, score in items:
                var = ctk.BooleanVar()
                # Màu của ô check box (fg_color) thành Xanh da trời mới
                chk = ctk.CTkCheckBox(scroll_frame, text=f"{content} (+{score}đ)", variable=var, font=("Arial", 14), 
                                      text_color=TEXT_COLOR_BLACK, 
                                      fg_color=COLOR_BLUE_DARKER, 
                                      hover_color=COLOR_BLUE_DARKER_HOVER, # Hover của xanh da trời đậm
                                      command=self.update_total_score)
        ...
        # Tiêu đề Thông tin bổ sung thành màu Xanh da trời mới
        ctk.CTkLabel(right_frame, text="THÔNG TIN BỔ SUNG", font=("Roboto Medium", 16), text_color=COLOR_BLUE_DARKER).pack(pady=(15, 10))
        ctk.CTkLabel(right_frame, text="Mã phiếu (Tự động):", font=("Arial", 13), text_color="gray").pack(anchor="w", padx=15)
        # Mã phiếu (text_color) thành Đỏ nhẹ
        ctk.CTkLabel(right_frame, text=self.auto_form_code, font=("Arial", 16, "bold"), text_color=COLOR_RED_LIGHT).pack(anchor="w", padx=15, pady=(0, 15))
        ...
        # Đảm bảo lệnh Hủy cũng clear biến theo dõi
        ctk.CTkButton(bottom_bar, text="Hủy bỏ", fg_color="#95a5a6", hover_color="#7f8c8d", width=100, 
                      command=lambda: (window.destroy(), setattr(self, 'active_child_window', None))).pack(side="left", padx=20)
        ...
        ctk.CTkButton(bottom_bar, text="GỬI PHIẾU ĐÁNH GIÁ", font=("Roboto Medium", 15), 
                      fg_color=COLOR_BLUE_DARKER, 
                      hover_color=COLOR_BLUE_DARKER_HOVER, 
                      width=200, height=40, command=lambda: self.submit_grading(window)).pack(side="right", padx=20)
    ...
    def submit_grading(self, window):
        ...
        if messagebox.askokcancel("Xác nhận nộp", msg, parent=window):
            ...
            if success:
                ...
                window.destroy()
                setattr(self, 'active_child_window', None)
                self.deselect_all()
                self.auto_form_code = f"MP{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
            else:
    ...
    def open_notifications(self):
        ...
        # Tiêu đề Danh sách thông báo thành màu Xanh da trời
        ctk.CTkLabel(window, text="DANH SÁCH THÔNG BÁO", font=("Roboto Medium", 22), text_color=COLOR_BLUE_TITLE).pack(pady=20)
        ...
        def view_detail(event=None):
            ...
            detail.transient(window) # Thiết lập cửa sổ chi tiết là con của cửa sổ thông báo
        ...
        # Nút Xem chi tiết nội dung thành màu Xanh da trời
        ctk.CTkButton(window, text="👁 Xem chi tiết nội dung", font=("Arial", 14, "bold"), 
                      fg_color=COLOR_BLUE_TITLE, hover_color="#00a3d6", 
