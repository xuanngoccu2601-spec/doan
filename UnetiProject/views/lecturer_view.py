...
# --- BẢNG MÀU MODERN UI (Đồng bộ với Sinh viên) ---
...
class LecturerDashboard(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # Logo / Tiêu đề
        ctk.CTkLabel(sidebar, text="GV. DASHBOARD", font=("Roboto Medium", 22), text_color=COLOR_TEXT_SIDEBAR).pack(pady=(40, 50))
    ...
    def setup_treeview_style(self):
        ...
        style.configure("Treeview", font=("Arial", 12), rowheight=35, 
                        background="white", fieldbackground="white", foreground="#2c3e50", borderwidth=0)
        style.map("Treeview", background=[('selected', COLOR_ACCENT)], foreground=[('selected', 'white')])

    # =========================================================================
    # TRANG CHỦ (DASHBOARD GRID)
    # =========================================================================
    def render_home(self):
        ...
        ctk.CTkLabel(welcome_frame, text=f"XIN CHÀO, {self.current_lecturer_name.upper()}", 
                     font=("Roboto Medium", 24), text_color=COLOR_TEXT_MAIN).pack(side="left", pady=(30, 0))
        ...
        funcs = [
            {"text": "Phiếu chờ duyệt", "icon": "📝", "cmd": self.open_pending_forms, "color": "#e67e22"},
            {"text": "Danh sách lớp", "icon": "📋", "cmd": self.open_class_list, "color": "#3498db"},
            {"text": "Thống kê lớp", "icon": "📊", "cmd": self.open_class_statistics, "color": "#9b59b6"},
            {"text": "Lịch sử duyệt", "icon": "🕒", "cmd": self.open_approval_history, "color": "#2ecc71"},
            {"text": "Thống kê Khoa", "icon": "🏢", "cmd": self.open_dept_statistics, "color": "#34495e"},
    ...
    def render_profile(self):
        ...
        ctk.CTkLabel(self.content_frame, text="HỒ SƠ GIẢNG VIÊN", font=("Roboto Medium", 26), text_color=COLOR_TEXT_MAIN).pack(anchor="w", padx=30, pady=(0, 20))
        
        info_card = ctk.CTkFrame(self.content_frame, fg_color="white", corner_radius=15)
        info_card.pack(fill="both", expand=True, padx=50, pady=10)
    ...
    def render_account(self):
        ...
        ctk.CTkLabel(self.content_frame, text="CÀI ĐẶT TÀI KHOẢN", font=("Roboto Medium", 26), text_color=COLOR_TEXT_MAIN).pack(anchor="w", padx=30, pady=(0, 20))
    ...
    def open_pending_forms(self):
        ...
        ctk.CTkLabel(window, text="DANH SÁCH PHIẾU CHỜ DUYỆT", font=("Roboto Medium", 20), text_color=COLOR_SIDEBAR).pack(pady=20)
        ...
        ctk.CTkButton(window, text="XEM CHI TIẾT & DUYỆT", fg_color=COLOR_ACCENT, hover_color=COLOR_HOVER, 
    ...
    def open_review_dialog(self, basic_info, details, tree, row_id, parent_window):
        ...
        dialog.attributes("-topmost", True); dialog.configure(fg_color="white")
        
        def on_close():
            parent_window.attributes("-topmost", True); dialog.destroy()
        ...
        # Tiêu chí
        ctk.CTkLabel(dialog, text="Các mục sinh viên đã chọn:", font=("Roboto Medium", 14), text_color=COLOR_SIDEBAR, anchor="w").pack(fill="x", padx=30, pady=(15, 5))
        ...
        ctk.CTkButton(btn_frame, text="DUYỆT PHIẾU", fg_color=COLOR_SUCCESS, width=150, height=40, hover_color="#2ecc71", command=lambda: confirm("Đã duyệt")).pack(side="right")
    ...
    def open_class_list(self):
        ...
        ctk.CTkLabel(window, text="DANH SÁCH SINH VIÊN QUẢN LÝ", font=("Roboto Medium", 20), text_color=COLOR_SIDEBAR).pack(pady=20)
        self.setup_treeview_style()
    ...
    def open_class_statistics(self):
        ...
        ctk.CTkLabel(window, text="THỐNG KÊ LỚP", font=("Roboto Medium", 22), text_color=COLOR_SIDEBAR).pack(pady=15)
        ...
        ctk.CTkButton(input_frame, text="Xem thống kê", command=load_stats, width=120, height=35, fg_color=COLOR_ACCENT).pack(side="left")
    ...
    def open_approval_history(self):
        ...
        ctk.CTkLabel(window, text=f"LỊCH SỬ DUYỆT BÀI", font=("Roboto Medium", 20), text_color=COLOR_SIDEBAR).pack(pady=20)
        self.setup_treeview_style()
    ...
    def open_dept_statistics(self):
        ...
        ctk.CTkLabel(window, text="THỐNG KÊ TOÀN KHOA", font=("Roboto Medium", 22), text_color=COLOR_SIDEBAR).pack(pady=20)
    ...
    def open_notifications(self):
        ...
        ctk.CTkLabel(window, text="THÔNG BÁO DÀNH CHO GIẢNG VIÊN", font=("Roboto Medium", 20), text_color=COLOR_SIDEBAR).pack(pady=20)
    ...
    def open_guide(self):
        ...
        ctk.CTkLabel(window, text="HƯỚNG DẪN GIẢNG VIÊN", font=("Roboto Medium", 20), text_color=COLOR_SIDEBAR).pack(pady=20)
